...
public class RequestManager {
    ...
    public static MediaType MEDIA_OBJECT_STREAM = MediaType.parse("application/octet-stream");

    //    private static final String BASE_URL = "http://xxx.com/openapi";//请求接口根地址
    private static final String BASE_URL = "http://192.168.253.13";//请求接口根地址
     ...
     */
    public <T> void downLoadFile(String fileUrl, String saveName, final String destFileDir, final ReqCallBack<T> callBack) {
        final String fileName = saveName;
     ...
     * @param destFileDir 存储目标目录
     */
    public <T> void downLoadFile(String fileUrl, String saveName, final String destFileDir, final ReqProgressCallBack<T> callBack) {
        final String fileName = saveName;
        final File file = new File(destFileDir, fileName);
        if (file.exists()) {
            successCallBack((T) file, callBack);
